...
def subimage(image):                       
   ...
   for (markerCorner,markerId) in zip (corners1,ids1):
       ...
       center=(tl+br)/2
       matrix = cv2.getRotationMatrix2D( center=center, angle=0, scale=1 )
...
def crop(img):
    ...
    if len(corners) > 0 :
       ...
       for(markerCorner,markerId) in zip(corners,ids):
           ...
           cropped_image = img[78:515, 78:515]
           cv2.imshow("cropped",cropped_image)
           cv2.imwrite("Ha_cropped.jpg",cropped_image)
           cv2.waitKey(0)
...
img3 = cv2.imread("D:\RONY\IIT Dhanbad\Atulya\My Project\Ha.jpg")
